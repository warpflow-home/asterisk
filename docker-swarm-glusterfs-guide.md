KeepalivedとSwarm Ingress（ルーティングメッシュ）を組み合わせた構成で、ドキュメント全体を書き直しました。

Asteriskはこれまで通り物理LANに直接繋がる `192.168.1.200` を維持し、その他の全サービスは共有の仮想IP（VIP） `192.168.1.240` を使ってポート番号でアクセスを振り分ける、より堅牢でスマートな設計になっています。
...
### 4-2. macvlan ネットワークの作成（全ノードで同一コマンドを実行）

> ⚠️ Asterisk専用の `192.168.1.200` 1つのみを割り当てる設定に変更しています。
> ⚠️ Swarmのスタックから参照できるよう、必ず `--scope swarm` を指定してください。

```bash
IFACE=$(ip route | grep default | awk '{print $5}')

# macvlan ネットワーク作成（全ノードで実行）
...
ここはほぼ元のままです。Asteriskは `192.168.1.200` を専有します。
...
フェーズ4 — ネットワーク (VIP + macvlan + overlay)
  ...
  ☐ swarm-macvlan 作成 (--scope swarm を指定し、IP範囲は 192.168.1.200/32)
  ☐ macvlan-host インターフェース設定 (192.168.1.200へのルート)

フェーズ5, 6, 7 — サービス
  ☐ Portainer スタックのデプロイ (overlay, Ingressポート)
  ☐ Asterisk スタックのデプロイ (Portainer経由、Githubから読み込み)
...
いかがでしょうか？こちらのKeepalivedを使った構成で、まずは `ping 192.168.1.240` が通るか試してみますか？不明点があればすぐにお答えします！
